...
//        wadFile += "DOOM1.WAD";
        ...
        try {
            BufferedImage textureTmp = ImageIO.read(View.class.getResourceAsStream("/res/texture.jpg"));
    ...
    private void fixedUpdate() {
        ...
        player.height += 0.2 * (targetHeight - player.height) + 0.5 * Math.sin(0.05 * player.y) + 0.5 * Math.cos(0.05 * player.x);;
    }
    ...
    protected void draw(Graphics2D g2d) {
        ...
        g2d.drawImage(offscreenB, 0, 0, null);
...
//        g.drawLine(p0.x, p0.y, p1.x, p1.y);
    ...
    private void draw3DWall(Graphics2D g, Seg seg) {
        ...
        double textureX0 = seg.offset + player.textureOffsetX;
        double textureX1 = seg.offset + seg.length + player.textureOffsetX;
...
//        offscreenG2D.drawString("" + (segDrawOrder++), sx, sy);
        ...
        int frontCeil = seg.frontSector.ceilingHeight - (int) player.height;
        int frontFloor = seg.frontSector.floorHeight - (int) player.height;
        ...
        if (seg.isPortal) {
            backCeil = seg.backSector.ceilingHeight - (int) player.height;
            backFloor = seg.backSector.floorHeight - (int) player.height;
        ...
        for (int clip = 0; clip < cbufferResult.size(); clip += 2) {
            ...
            for (int x = scrLeft; x <= scrRight; x++) {
                ...
                double middleTextureY = player.textureOffsetY;
                if (middleMinY < lowerOcclusion[x]) {
                    double dif = lowerOcclusion[x] - middleMinY;
                    middleTextureY = dif * middleTextureYStep + player.textureOffsetY;
                ...
                if (!seg.isPortal) {
                    ...
                    for (int y = middleMinY; y < middleMaxY; y++) {
                        int ttx = (int) (currentTextureX / currentZInv) & 0x7fffffff;
                        int tx = ttx & 0b00111111;
                        int ty = (int) (middleTextureY) & 0b00111111;
                        offscreenBData[x + y * 800] = textureData[ty * texture.getWidth() + tx];
                        middleTextureY += middleTextureYStep;
                    }
                }

                //floor
                if (middleMaxY < upperOcclusion[x]) {
                    offscreenBG2D.setColor(seg.color);
                    int ceilMin = (int) Math.max(lowerOcclusion[x], middleMaxY);
                    if (upperOcclusion[x] > ceilMin) {                    
//                        int c = seg.color.getRGB();
//                        for (int y = ceilMin; y < upperOcclusion[x]; y++) {
//                            offscreenBData[x + y * 800] = c;
//                        }
//                        for (int y = ceilMin; y < upperOcclusion[x]; y++) {
//                            offscreenBData[x + y * 800] = c;
//                        }
...
//                        }
                    ...
                    //else if (floorVisplane.getPreviousX() >= 0) {
                    //    floorVisplane.finish();
                    //    floorVisplane.draw(g, 1, 0, 0, seg.color);
                    //    floorVisplane.reset();
                    //}
                    upperOcclusion[x] = middleMaxY;
                }
                ...
                if (hasLowerWall) {
                    ...
                    double lowerTextureY = player.textureOffsetY;
                    if (lowerMinY < lowerOcclusion[x]) {
                        double dif = lowerOcclusion[x] - lowerMinY;
                        lowerTextureY = dif * lowerTextureYStep + player.textureOffsetY;
                    ...
                    for (int y = lowerMinY; y < lowerMaxY; y++) {
                        int ttx = (int) (currentTextureX / currentZInv) & 0x7fffffff;
                        int tx = ttx & 0b00111111;
                        int ty = (int) (lowerTextureY) & 0b00111111;
                        offscreenBData[x + y * 800] = textureData[ty * texture.getWidth() + tx];
                ...
                if (middleMinY > lowerOcclusion[x]) {
                    ...
                    if (lowerOcclusion[x] < ceilMax) {
//                        int c = seg.color.getRGB();
//                        for (int y = lowerOcclusion[x]; y < ceilMax; y++) {
//                            offscreenBData[x + y * 800] = c;
//                        }
//                        for (int y = lowerOcclusion[x]; y < ceilMax; y++) {
//                            offscreenBData[x + y * 800] = c;
...
//                            offscreenBData[x + y * 800] = c;
//                        }
                        //offscreenBG2D.drawLine(x, ceilMax, x, lowerOcclusion[x]);
                        //offscreenBG2D.drawLine(x, ceilMax, x, lowerOcclusion[x]);
                        //offscreenBG2D.drawLine(x, ceilMax, x, lowerOcclusion[x]);
                        ceilVisplane.addColumn(x, lowerOcclusion[x], ceilMax);

                    }
                    //else if (ceilVisplane.getPreviousX() >= 0) {
                    //    ceilVisplane.finish();
                    //    ceilVisplane.draw(g, 1, 0, 0, seg.color);
                    //    ceilVisplane.reset();
                    //}
                ...
                if (hasUpperWall) {
                    ...
                    double upperTextureY = player.textureOffsetY;
                    if (upperMinY < lowerOcclusion[x]) {
                        double dif = lowerOcclusion[x] - upperMinY;
                        upperTextureY = dif * upperTextureYStep + player.textureOffsetY;
                    ...
                    for (int y = upperMinY; y < upperMaxY; y++) {
                        int ttx = (int) (currentTextureX / currentZInv) & 0x7fffffff;
                        int tx = ttx & 0b00111111;
                        int ty = (int) (upperTextureY) & 0b00111111;
                        offscreenBData[x + y * 800] = textureData[ty * texture.getWidth() + tx];
            ...
            if (floorVisplane.getPreviousX() >= 0) {
                ...
                seg.frontSector.drawFloorTest(offscreenBData, player, floorVisplane);
            }
            if (ceilVisplane.getPreviousX() >= 0) {
                ceilVisplane.finish();
                //ceilVisplane.draw(offscreenBG2D, 1, 0, 0, seg.color);
                seg.frontSector.drawCeilingTest(offscreenBData, player, ceilVisplane);
            }
        }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            View mapViewer = new View();
            mapViewer.setPreferredSize(new Dimension(800, 600));

            JFrame frame = new JFrame();
            frame.setTitle("DOOM WAD Map Viewer Test #3");
