...
//        wadFile += "DOOM1.WAD";
    ...
    private void fixedUpdate() {
        Util.update();
        player.fixedUpdate();
        double targetHeight = 48 + getCurrentPlayerHeight();
        player.height += 0.2 * (targetHeight - player.height);
        player.height += 1.0 * Math.sin(0.04 * player.y) + 1.0 * Math.cos(0.04 * player.x) - 1.0;
    }
...
//        offscreenG2D.drawString("" + (segDrawOrder++), sx, sy);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            frame.setTitle("DOOM WAD Map Viewer Test #4");
