...
public class View extends Canvas implements Runnable {
    ...
    private final CBufferNode cbuffer = new CBufferNode(0, 799);
...
//        System.out.println("loading wad: " + wadFile);
        ...
        String wadFile = "D:/games/doom_dos_win/Doom/game/DOOM.WAD";
        try {
        ...
        for (Thing thing : WADLoader.things) {
            if (thing.type == 9) { // https://doomwiki.org/wiki/Shotgun_guy
                Enemy enemy = new Enemy(thing);
                enemies.add(enemy);
            }
        }
    ...
    protected void draw(Graphics2D g2d) {
        ...
        enemies.forEach(enemy -> enemy.draw(g2d, player));
...
//        offscreenG2D.drawString("" + (segDrawOrder++), sx, sy);
        ...
        double frontCeil = (seg.frontSector.ceilingHeight - player.heightWalk);
        double frontFloor = (seg.frontSector.floorHeight - player.heightWalk);
        ...
        if (seg.isPortal) {
            backCeil = (seg.backSector.ceilingHeight - player.heightWalk);
            backFloor = (seg.backSector.floorHeight - player.heightWalk);
...
//                    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            frame.setTitle("DOOM WAD Map Viewer Test #5");
