...
public class Picture {
    ...
    public void flip() {
        ...
        imageData = ((DataBufferInt) imageTmp.getRaster().getDataBuffer()).getData();

        imageTmp.getGraphics().drawImage(image, width, 0, -width, height, null);
        image = imageTmp;
    }
    ...
    public void fixRGB() {
        for (int y = 0; y < image.getHeight(); y++) {
            for (int x = 0; x < image.getWidth(); x++) {
                ...
                int indexedColor = color & 0xff;
                int rgb = Palette.colors[indexedColor];
