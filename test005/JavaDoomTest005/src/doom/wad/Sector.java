...
public class Sector {
    ...
    public void drawFloorTest(int[] offscreen3DData, Player player, Visplane visplane, Sector sector) {
        ...
        for (int y = minY; y < maxY; y++) {
            ...
            double z = planeDistance * (-floorHeight + player.heightWalk) * FLOOR_Y_INV[y];
            ...
            for (int i = 0; i < visplane.horizontalSpanIndex[y] - 1; i += 2) {
                int visplaneLeft = visplane.horizontalSpans[y][i + 0];
                int visplaneRight = visplane.horizontalSpans[y][i + 1];

                double worldX = leftX + dx * visplaneLeft;
                double worldY = leftY + dy * visplaneLeft;
                int pixelIndex = y * 800 + visplaneLeft;
                for (int screenX = visplaneLeft; screenX <= visplaneRight; screenX++) {
    ...
    public void drawCeilingTest(int[] offscreen3DData, Player player, Visplane visplane, Sector sector) {
        ...
        if (ceilingPic != null) {
            ...
            double textureOffsetX = ceilingTextureWidth * (normPlayerAngle / (Math.PI * 0.5));
            ...
            for (int y = visplane.maxY; y >= visplane.minY; y--) {
                ...
                for (int spanIndex = 0; spanIndex < visplane.horizontalSpanIndex[y] - 1; spanIndex += 2) {
                    int visplaneLeft = visplane.horizontalSpans[y][spanIndex];
                    int visplaneRight = visplane.horizontalSpans[y][spanIndex + 1];

                    int pixelIndex = y * 800 + visplaneLeft;
                    int textureY = (int) (y * dy) << 8;
                    double textureX = dx * visplaneLeft - textureOffsetX;
                    for (int screenX = visplaneLeft; screenX <= visplaneRight; screenX++) {
            ...
            for (int y = maxY; y >= minY; y--) {
                ...
                double z = -planeDistance * (-ceilingHeight + player.heightWalk) * CEIL_Y_INV[y];
                int[] colorMap = ColorMap.getFloorCeilingColorMap(z, sector);
                double px = playerDir.x * z + player.x;
                double py = playerDir.y * z + player.y;
