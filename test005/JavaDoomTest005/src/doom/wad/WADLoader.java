...
public class WADLoader {
    ...
    public static Blockmap extractBlockmap(Directory lump) {
        System.out.println("extracting blockmap " + lump);
        ByteBuffer lumpData = lump.data;
        lumpData.position(0);
        int gridOriginX = lumpData.getShort();
        int gridOriginY = lumpData.getShort();
        Point gridOrigin = new Point(gridOriginX, gridOriginY);
        int cols = lumpData.getShort();
        int rows = lumpData.getShort();
        int[] offsets = new int[cols * rows];
        for (int i = 0; i < offsets.length; i++) {
            offsets[i] = lumpData.getShort();
        }
        Linedef[][][] blocks = new Linedef[rows][cols][];
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                lumpData.position(2 * offsets[row * cols + col]);
                List<Linedef> linedefsTmp = new ArrayList<>();
                int linedefIndex;
                while ((linedefIndex = lumpData.getShort()) >= 0) {
                    Linedef linedef = linedefs.get(linedefIndex);
                    linedefsTmp.add(linedef);
                }
                blocks[row][col] = linedefsTmp.toArray(new Linedef[0]);
            }
        }
        return new Blockmap(gridOrigin, cols, rows, blocks);
    }
    ...
    public static void loadMap(String mapName) {
        ...
        // BLOCKMAP
        Directory blockmapLump = getLumpByName(mapName, "BLOCKMAP");
        blockmap = extractBlockmap(blockmapLump);
    ...
    public static double getMapFloorHeight(double x, double y) {
